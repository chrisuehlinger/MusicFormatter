...
public class Note{
    ...
    public java.lang.String convertToGuido(int divisions)
    {
    	java.lang.String guidoNote = "";
    	if(notations != null)
    		for(Notations n: notations)
    			for(Object notation: n.getTiedOrSlurOrTuplet())
    				if(notation instanceof Fermata)
    					guidoNote += " \\fermata( ";
    				else if(notation instanceof Articulations)
    					for(JAXBElement<?> articulation: ((Articulations)notation).getAccentOrStrongAccentOrStaccato()){
    						if(articulation.getName().getLocalPart().compareTo("staccato") == 0)
    							guidoNote += " \\stacc( ";
    						else if(articulation.getName().getLocalPart().compareTo("accent") == 0)
    							guidoNote += " \\accent( ";
    						else if(articulation.getName().getLocalPart().compareTo("tenuto") == 0)
    							guidoNote += " \\ten( ";
    					}
    				else if(notation instanceof Technical)
    					for(JAXBElement<?> technical: ((Technical)notation).getUpBowOrDownBowOrHarmonic())
    						if(technical.getName().getLocalPart().compareTo("harmonic") == 0)
    							guidoNote += " \\harmonic(";
    	if(lyric != null)
    		for(Lyric l: lyric){
    			java.lang.String text ="";
        		Syllabic syllabic = null;
    			for(Object o: l.getElisionAndSyllabicAndText())
    				if(o instanceof TextElementData)
    					text = ((TextElementData)o).getValue();
    				else if(o instanceof Syllabic)
    					syllabic = (Syllabic)o;
    			
    			guidoNote += " \\lyrics<\"";
    			//if(syllabic == Syllabic.MIDDLE || syllabic == Syllabic.END)
    				//guidoNote += "-";
    			guidoNote += text;
    			//if(syllabic == Syllabic.MIDDLE || syllabic == Syllabic.BEGIN)
    				//guidoNote += "-";
    			guidoNote += "\">(";
    		}
    	...
    	if(rest != null)
    		...
    		if(pitch.getAlter() != null)
    			switch(pitch.getAlter().intValue()){
    			case 2: guidoNote = guidoNote + "##"; break;
    			case 1: guidoNote = guidoNote + "#"; break;
    			case -1: guidoNote = guidoNote + "&"; break;
    			case -2: guidoNote = guidoNote + "&&"; break;
    			}
    			
    		guidoNote = guidoNote + (pitch.getOctave()-3);
      	}else if(unpitched != null)
      	{
      		switch(unpitched.displayStep)
    		{
    		case A:
    			guidoNote+="a";
    			break;
    		...
    		guidoNote = guidoNote + (unpitched.displayOctave-3);
      	}
    	
    	if(duration != null)
    		guidoNote += "*" + duration.intValue() + "/" + (divisions*4);
    	
    	if(dot != null)
    		for(EmptyPlacement d: dot)
    			guidoNote += ".";
    		...
    		for(Notations n: notations)
    			for(Object notation: n.getTiedOrSlurOrTuplet())
    				if(notation instanceof Fermata)
    					...
    					for(JAXBElement<?> articulation: ((Articulations)notation).getAccentOrStrongAccentOrStaccato()){
    						if(articulation.getName().getLocalPart().compareTo("staccato") == 0
    								|| articulation.getName().getLocalPart().compareTo("accent") == 0
    								|| articulation.getName().getLocalPart().compareTo("tenuto") == 0)
    							guidoNote += " ) ";
    					}
    					else if(notation instanceof Technical)
    						for(JAXBElement<?> technical: ((Technical)notation).getUpBowOrDownBowOrHarmonic())
    ...
    public void flipStem(){
    	Stem s = new Stem();
		if(stem != null && stem.getValue() == StemValue.UP)
			s.setValue(StemValue.DOWN);
		else
			s.setValue(StemValue.UP);
		setStem(s);
    }
    
    public void addOrRemoveFermata(){
    	...
    	if(notations != null)
    		for(Notations n: notations)
    			for(Object notation: n.getTiedOrSlurOrTuplet())
    				if(notation instanceof Fermata){
    					hasFermata = true;
    					n.getTiedOrSlurOrTuplet().remove(notation);
    				}
    	
    	//Add Fermata if there was not one
    	if(!hasFermata) {
    		Fermata f = new Fermata();
    		Notations n =  new Notations();
    		n.tiedOrSlurOrTuplet = new ArrayList<Object>();
    		n.getTiedOrSlurOrTuplet().add(f);
    		if(notations == null)
    			notations = new ArrayList<Notations>();
    		notations.add(n);
    	}
    ...
    public void addOrRemoveArticulation(java.lang.String name){
    		...
    		for(Notations n: notations)
    			for(Object notation: n.getTiedOrSlurOrTuplet())
    				if(notation instanceof Articulations)
    					for(JAXBElement<?> articulation: ((Articulations)notation).getAccentOrStrongAccentOrStaccato())
    						if(articulation.getName().getLocalPart().compareTo(name) == 0){
    							hasArticulation = true;
    							((Articulations)notation).getAccentOrStrongAccentOrStaccato().remove(articulation);
    						}
    	
    	//Add Articulation if there was not one
    	if(!hasArticulation) {
    		JAXBElement<?> newArticulation = new JAXBElement(new QName(name), EmptyPlacement.class, new EmptyPlacement());
    		Articulations a = new Articulations();
    		a.accentOrStrongAccentOrStaccato = new ArrayList<JAXBElement<?>>();
    		a.getAccentOrStrongAccentOrStaccato().add(newArticulation);
    		Notations n =  new Notations();
    		n.tiedOrSlurOrTuplet = new ArrayList<Object>();
    		n.getTiedOrSlurOrTuplet().add(a);
    		if(notations == null)
    			notations = new ArrayList<Notations>();
    		notations.add(n);
    	}
    ...
    public void addOrRemoveTechnical(java.lang.String name){
    	boolean hasTechnical = false;
    	if(notations != null)
    		for(Notations n: notations)
    			for(Object notation: n.getTiedOrSlurOrTuplet())
    				if(notation instanceof Technical)
    					for(JAXBElement<?> technical: ((Technical)notation).getUpBowOrDownBowOrHarmonic())
    						if(technical.getName().getLocalPart().compareTo(name) == 0){
    							hasTechnical = true;
    							((Technical)notation).getUpBowOrDownBowOrHarmonic().remove(technical);
    						}
    	
    	//Add Articulation if there was not one
    	if(!hasTechnical) {
    		JAXBElement<?> newTechnical = new JAXBElement(new QName(name), EmptyPlacement.class, new EmptyPlacement());
    		Technical a = new Technical();
    		a.upBowOrDownBowOrHarmonic = new ArrayList<JAXBElement<?>>();
    		a.getUpBowOrDownBowOrHarmonic().add(newTechnical);
    		Notations n =  new Notations();
    		n.tiedOrSlurOrTuplet = new ArrayList<Object>();
    		n.getTiedOrSlurOrTuplet().add(a);
    		if(notations == null)
